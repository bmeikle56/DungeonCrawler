...
public class TreasureRoomController extends RoomController {
    ...
    @FXML
    private ProgressBar goblinBar;
    @FXML
    private Label goblinValue;
    @FXML
    ...
    public void showGoblinBar(double health) {
        goblinBar.setProgress(health);
    }
    public void showGoblinValue(int health) {
        goblinValue.setText(String.valueOf(health));
    }
    ...
    public void clickCollect(MouseEvent mouseEvent) throws IOException {
        if (Player.isDead()) {
            // player can die from an unlucky potion
        ...
        Lootable loot = ((TreasureRoom) room).getTreasure();
        if (loot instanceof Treasure) {
            Dungeon.addCoins(((Treasure) loot).amount());
        }
        loot.loot();

        checkLootStatus();
        System.out.println("Inventory: " + Inventory.print());
        announce.setText(loot.getDescription());
    ...
    public void clickFight(MouseEvent mouseEvent) throws IOException {
        Room room = Dungeon.getRoom();
        Goblin goblin = ((TreasureRoom) room).getMonster();
        ...
        if (goblin.isDead()) {
            Image image = new Image(getClass().getResourceAsStream("monsterImgs/deadGoblin.png"));
            gob1.setFitHeight(150);
            gob1.setFitWidth(200);
            gob1.setImage(image);
        }
        ...
        showStrength(Player.getStrength());
        showHealthValue((int) (Player.round(Player.getHealth(), 2) * 100));
        showGoblinBar(goblin.getHealth());
        showGoblinValue((int) (Player.round(goblin.getHealth(), 2) * 100));
    }
    @Override
    public void checkMonsterStatus() {
        Room room = Dungeon.getRoom();
        Goblin goblin = ((TreasureRoom) room).getMonster();
        Lootable treasure = ((TreasureRoom) room).getTreasure();
        if (goblin.isDead()) {
            fight.setVisible(false);
            collect.setVisible(true);
            deadLabel.setText("You killed the monster!");
            goblinBar.setVisible(false);
            goblinValue.setVisible(false);
