...
public class ShopRoomController extends RoomController {
    @FXML
    ...
    @FXML
    private ImageView serp;
    @FXML
    private ProgressBar serpentBar;
    @FXML
    private Label serpentValue;
    @FXML
    ...
    public void showSerpentBar(double health) {
        serpentBar.setProgress(health);
    }
    public void showSerpentValue(int health) {
        serpentValue.setText(String.valueOf(health));
    }
    ...
    public void clickTrade(MouseEvent mouseEvent) throws IOException {
        checkTradeStatus();
        loadShop(mouseEvent);
    }

    private void loadShop(MouseEvent mouseEvent) throws IOException {
    ...
    public void clickFight(MouseEvent mouseEvent) throws IOException {
        ...
        Serpent serpent = ((ShopRoom) room).getMonster();
        Player.attack(serpent);
        serpent.attack();
        deadLabel.setText("You got attacked!");

        checkPlayerStatus(mouseEvent);
        checkMonsterStatus();
        updatePlayer();
        showHealthBar(Player.getHealth());
        showStrength(Player.getStrength());
        showHealthValue((int) (Player.round(Player.getHealth(), 2) * 100));
        showSerpentBar(serpent.getHealth());
        showSerpentValue((int) (Player.round(serpent.getHealth(), 2) * 100));
    }

    public void checkPlayerStatus(MouseEvent mouseEvent) throws IOException {
        if (Player.isDead()) {
    ...
    public void checkMonsterStatus() {
        ...
        if (serpent.isDead()) {
            ...
            Image image = new Image(getClass().getResourceAsStream("monsterImgs/deadFoot.jpg"));
            deadLabel.setText("You killed the monster!");
            serpentValue.setVisible(false);
            serpentBar.setVisible(false);
            serp.setFitHeight(150);
            serp.setFitWidth(200);
            serp.setImage(image);
            checkLocation();
        } else {
